...
function SimulatorMode({ isPremium, openPaywall }) {
  ...
  const STORAGE_KEY = 'pickleball_rallies_played'; // localStorage保存キー
  ...
  // localStorage から累計ラリー数を読み込む(初期化時のみ)
  const [ralliesPlayed, setRalliesPlayed] = useState(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      return saved ? parseInt(saved, 10) : 0;
    } catch (e) {
      // localStorage使えない環境(プライベートモード等)は0で開始
      return 0;
    }
  });

  // ラリー数が変わるたびにlocalStorageに保存
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, String(ralliesPlayed));
    } catch (e) {
      // localStorage使えない環境では無視
    }
  }, [ralliesPlayed]);
